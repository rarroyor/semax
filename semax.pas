...
begin
  if not Assigned(PrevProc) then
     PrevProc := TWndProc({$ifdef cpu64} Windows.GetWindowLongPtr(Form.Handle, GWL_WNDPROC)
                          {$else}        Windows.GetWindowLong(Form.Handle, GWL_WNDPROC)
                          {$endif}); //, PtrInt(@DestroyWindowProc));                      
